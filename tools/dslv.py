...
class ItemArtist:
    ...
    def _check_boxes(self, state):
        ...
        if self.position == state.src:
            ...
            if self.src_box is not None:
                self.src_box.remove()
                self.src_box = None
            if self.exp_box is not None:
                self.exp_box.remove()
                self.exp_box = None
...
class StateArtist:
    ...
    def update(self, state):
        ...
        for item in self.items:
            if item.in_state(state):
                item.update(item.position, state)
                updated_pos.append(item.position)
            else:
                to_recycle.append(item)

        pos_set = set(state.positions)
        updated_pos_set = set(updated_pos)

        unupdated_pos_set = pos_set.symmetric_difference(updated_pos_set)
        for position in unupdated_pos_set:
            if len(to_recycle) != 0:
                item = to_recycle.pop(-1)
...
if __name__ == "__main__":
    states = get_states("dslv_state.txt")
